...
namespace VideoScheduler.Data
{
    public class SchedulePlanner
    {
        ...
        public void RefreshForecast(string? changedRunId = null)
        {
            var now = DateTime.Now;
            DateTime safeStartTime = now;
            List<PlaylistItem> keptItems = new();

            // 1. Identify Cutoff Point
            if (CurrentPlaylist != null && CurrentPlaylist.Any())
            {
                ...
                if (currentItem != null)
                {
                    // Always regenerate from the end of the current item to ensure
                    // schedule changes (deletions/edits) are reflected immediately.
                    safeStartTime = currentItem.EstimatedStartTime.AddSeconds(currentItem.DurationSec);
                    keptItems = CurrentPlaylist.Where(i => i.EstimatedStartTime < safeStartTime).ToList();
                }
            }
